...
public class ApngFrameDecode {
    ...
    Bitmap createFrameBitmap(final int frameIndex) {
        ...
        complexBitmap = handleBlendingOperation(offsetX, offsetY, blendOp, currentBitmap, previousBitmap, frameIndex);
    ...
    private Bitmap handleBlendingOperation(
            ...
            Bitmap frameBitmap, Bitmap baseBitmap, int frameIndex) {
...
//            Log.d("zlei","blendOp:"+blendOp);
            if (blendOp == PngChunkFCTL.APNG_BLEND_OP_SOURCE) {
                //@20170813 修正APNG在裁剪后由于offset 导致的没有清除边缘，导致上一帧残留问题，
                //对性能的影响取决于图片每一帧优化后的尺寸和图片内容。
                //一般影响可以忽略
                //@20170903 修正失败，还原部分代码，可能导致上一帧图片部分残留。需要测试
                canvas.clipRect(offsetX, offsetY, offsetX + frameBitmap.getWidth(), offsetY + frameBitmap.getHeight());
//                Log.d("zlei","offsetX:"+offsetX);
//                Log.d("zlei","offsetY:"+offsetY);

//                Log.d("zlei","baseWidth:"+apngDrawable.baseWidth);
//                Log.d("zlei","baseHeight:"+apngDrawable.baseHeight);
//                if(frameIndex != fctlArrayList.size() -1) {
...
//        Log.d("zlei","frameBitmap getWidth:"+frameBitmap.getWidth());
//        Log.d("zlei","frameBitmap getHeight:"+frameBitmap.getHeight());
    ...
    private Bitmap handleDisposeOperation(int frameIndex) {
        ...
        switch (disposeOp) {
            ...
            case PngChunkFCTL.APNG_DISPOSE_OP_BACKGROUND:
                ...
                if (frameWHMap.containsKey(frameIndex - 1)) {

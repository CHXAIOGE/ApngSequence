...
import static com.xiaoge.graphics.apngsequence.apngdrawable.ApngImageUtils.reconfigure;
...
public class ApngBitmapCache {
    ...
    Bitmap getReuseBitmap(int width, int height) {
        ...
        while (iter.hasNext()) {
            ...
            if (bitmap != null && BitmapCompat.getAllocationByteCount(bitmap) >= reuseSize) {
                iter.remove();
                if (bitmap.getWidth() != width || bitmap.getHeight() != height) {
                    bitmap = reconfigure(bitmap, width, height, Bitmap.Config.ARGB_8888);
                }
                bitmap.eraseColor(0);
                return bitmap;
            }

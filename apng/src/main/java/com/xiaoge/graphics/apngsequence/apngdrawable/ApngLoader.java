...
public class ApngLoader {
    ...
    public static void loadImageFile(final String uri, final ImageView imageView, final ApngImageLoadingListener listener) {
        getInstance().excutor.execute(new Runnable() {
            @Override
            public void run() {
                ApngImageUtils.Scheme urlType = ApngImageUtils.Scheme.ofUri(uri);
                ...
                Drawable drawable = ApngImageUtils.bitmapToDrawable2(uri, imageView, decodeBitmap);;
                // 将结果通知ui业务层
                final Drawable finalDrawable = drawable;
                getInstance().uiHandler.post(new Runnable() {
                    @Override
